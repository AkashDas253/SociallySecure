...
# Load the OCR_ENGINE environment variable
from dotenv import load_dotenv
load_dotenv() 
ocr_engine = os.getenv('OCR_ENGINE', 'TESSERACT').upper()
print(f"OCR_ENGINE: {ocr_engine}")  # Debug print statement
...
def extract_text_from_image(image):
    ...
    if ocr_engine == 'EASYOCR':
        ...
        # Perform OCR using EasyOCR on the NumPy array
...
# 2. Function to find the longest common substring between two strings
...
# 3. Function to find matching credentials
...
# 4. Function to draw bounding boxes on the image
def draw_bounding_boxes(image, extracted_text_dict, matching_results, match_threshold=50):
    ...
    for text, data in extracted_text_dict.items():
        ...
        if matching_results.get(text, {}).get("match_percentage", 0) > match_threshold:
...
# 5. Combined function that performs all steps
